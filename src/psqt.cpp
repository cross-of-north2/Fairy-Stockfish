...
// structure offsets control

static_assert( Bonus[KNIGHT - 1][0][0] == 0 );
static_assert( Bonus[KNIGHT][0][0] != 0 );
static_assert( Bonus[BISHOP][0][0] != 0 );
static_assert( Bonus[ROOK][0][0] != 0 );
static_assert( Bonus[QUEEN][0][0] != 0 );
static_assert( Bonus[QUEEN + 1][0][0] == 0 );

static_assert( Bonus[ARCHBISHOP - 1][0][0] == 0 );
static_assert( Bonus[ARCHBISHOP][0][0] != 0 );
static_assert( Bonus[CHANCELLOR][0][0] != 0 );
static_assert( Bonus[AMAZON][0][0] != 0 );
static_assert( Bonus[AMAZON + 1][0][0] == 0 );

static_assert( Bonus[MUSKETEER_CANNON - 1][0][0] == 0 );
static_assert( Bonus[MUSKETEER_CANNON][0][0] != 0 );
static_assert( Bonus[LEOPARD][0][0] != 0 );
static_assert( Bonus[SPIDER][0][0] != 0 );
static_assert( Bonus[UNICORN][0][0] != 0 );
static_assert( Bonus[HAWK][0][0] != 0 );
static_assert( Bonus[MUSKETEER_ELEPHANT][0][0] != 0 );
static_assert( Bonus[FORTRESS][0][0] != 0 );
static_assert( Bonus[FORTRESS + 1][0][0] == 0 );
  ...
  for (PieceType pt = PAWN; pt <= KING; ++pt)
  {
      ...
      for (Square s = SQ_A1; s <= SQ_MAX; ++s)
      {
          ...
          psq[ pc][s] = score + (  pt == PAWN  ? PBonus[std::min(r, RANK_8)][std::min(file_of(s), FILE_H)]
                                 ...
                                 : (
                                     pt <= QUEEN
                                     || ( pt >= ARCHBISHOP && pt <= AMAZON )
                                     || ( pt >= MUSKETEER_CANNON && pt <= FORTRESS )
